...
Session::Session(asio::ip::tcp::socket socket,
  ...
  if (!codec_)  codec_  = std::make_shared<IdentityCodec>();
  if (!logger_) logger_ = std::make_shared<NullLogger>();
}

void Session::start(OnMessage on_msg, OnDisconnect on_disc) {
  asio::dispatch(strand_,
    [self = shared_from_this(), on_msg = std::move(on_msg), on_disc = std::move(on_disc)]() mutable {
...
void Session::do_read() {
  ...
  socket_.async_read_some(
    ...
    asio::bind_executor(strand_,
      [self = shared_from_this()](const ErrorCode& ec, std::size_t n) {
        ...
        try {
          ...
          for (auto& f : frames) {
            ...
            if (self->on_msg_) self->on_msg_(self, std::move(msg));
          }
...
void Session::do_write() {
  if (closed_) return;
  if (write_q_.empty()) { writing_ = false; return; }
  ...
  asio::async_write(
    ...
    asio::bind_executor(strand_,
      [self = shared_from_this(), data](const ErrorCode& ec, std::size_t /*n*/) {
        if (ec) { self->fail(ec); return; }
...
void Session::arm_read_timeout() {
  if (cfg_.read_timeout.count() <= 0) return;

  read_timer_.expires_after(cfg_.read_timeout);
  read_timer_.async_wait(asio::bind_executor(strand_,
...
void Session::arm_write_timeout() {
  if (cfg_.write_timeout.count() <= 0) return;
...
void Session::fail(const ErrorCode& ec) {
  ...
  if (on_disc_) on_disc_(shared_from_this(), ec);
}
