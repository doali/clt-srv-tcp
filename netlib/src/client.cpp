...
void TcpClient::connect(asio::ip::tcp::endpoint ep, Session::OnMessage on_msg, Session::OnDisconnect on_disc) {
  endpoint_ = ep;
...
void TcpClient::do_connect() {
  ...
  sock->async_connect(endpoint_,
    [this, sock](const ErrorCode& ec) {
      if (ec) {
        ...
        if (user_on_disc_) user_on_disc_(session_, ec);
        return;
      }
      ...
      s->start(
        [this](std::shared_ptr<Session> ss, ByteVec&& msg) {
          if (user_on_msg_) user_on_msg_(std::move(ss), std::move(msg));
        },
...
void TcpClient::on_session_disconnect(std::shared_ptr<Session> s, const ErrorCode& ec) {
  if (user_on_disc_) user_on_disc_(s, ec);

  if (disconnecting_) return;
  if (!cfg_.auto_reconnect) return;
  if (cfg_.retry.max_attempts != 0 && attempt_ >= cfg_.retry.max_attempts) return;
